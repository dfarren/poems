...
flags.DEFINE_float('keep_prob', 0.75, 'probability of keeping data after dropout')
...
# Cross-entropy loss
def cal_cross_entropy(pred, labels):
...
# Model
def multilayer_perceptron(X, y, keep_prob):
    with tf.variable_scope('train'):
        ...
        pred = nn_layer(layer_1, FLAGS.n_hidden_1, FLAGS.n_classes, 'output', act=None)
        loss = cal_cross_entropy(pred, y)

        labels_pred = nn_layer(layer_1, FLAGS.n_hidden_1, FLAGS.n_classes, 'output', act=tf.nn.softmax)
...
# Loss & optimizer
xentropy_loss, labels_pred = multilayer_perceptron(X, y, keep_prob)
opt_operation = tf.train.AdamOptimizer(FLAGS.learning_rate).minimize(xentropy_loss)
precision, recall, f1, acc = evaluate(labels_pred, y)
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    for epoch in xrange(FLAGS.num_epochs):
        ...
        for step in xrange(number_batch + 1):
            ...
            if step % FLAGS.display_step == 0:
                ...
                ops_train = [xentropy_loss, precision, recall, f1, acc] 
                x_loss, pre, rec, f1_score, accuracy = \
                ...
                ops_val = [xentropy_loss, f1, acc]
                val_xentropy, val_f1, val_acc = sess.run(ops_val, feed_dict = {X: val_X,
                                                                               y: val_y,
                                                                               keep_prob: 1})
    ...
    ops_test = [xentropy_loss, precision, recall, f1, acc]
    test_loss, test_pre, test_rec, test_f1, test_acc = sess.run(ops_test, feed_dict = {X: test_X,
                                                                                       y: test_y,
                                                                                       keep_prob: 1})
